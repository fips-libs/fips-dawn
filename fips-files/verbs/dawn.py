...
def bootstrap(fips_dir):
    ...
    write_build_args(fips_dir, 'Release', ['is_debug=false', 'dawn_complete_static_libs=true'])
    log.info('>> generating debug build files')
    gn(fips_dir, ['gen', 'out/Debug'])
    write_build_args(fips_dir, 'Debug', ['is_debug=true', 'dawn_complete_static_libs=true'])
    out_dir = get_out_dir(fips_dir)
